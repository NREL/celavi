...
def liaison_lci(
    ...
    state_from_celavi =  us_state_codes

    for index,row in liaison_process_bridge.iterrows():
        # This is the method of connecting with CELAVI. Not using this since we want to debug the activities
        selected_process = liaison_process_bridge[liaison_process_bridge['Activities'] == row['Activities']]
        ...
        if (len(selected_process) == 1):
            ...
            for st in state_from_celavi: 
                ...
                res_df['stage']   = process_in_ecoinvent_for_lca_from_celavi
                res_df['material']  = process_in_ecoinvent_for_lca_from_celavi
                res_df['route_id']   = route_id
                res_df['state'] = "US-"+st

                lca_db = res_df
                lca_db['year'] = lca_db['year'].astype(int)
                lca_db['value'] = lca_db['value']/1
                lca_db = lca_db.drop_duplicates()
                lca_db.to_csv(
                    filename_shortcut+"_2000_all.csv",
                    mode="a",
                    index=False,
                    header=False,
                )
                res_df.to_csv('2000_checked_to_be_deleted.csv',mode='a', index=False)
                print(str(time.time()-tim00),' seconds for one lca calculation of ', str(year_from_celavi), process_in_ecoinvent_for_lca_from_celavi,"US-"+st) 
