...
def runner(tech_matrix, F,i,j,k,l,m,n,p,final_demand_scaler,lci_dict,lciflow_dict):
               tim0 = time.time()  
               res = pd.DataFrame()
               res2 = pd.DataFrame()                        
               ...
               if res2.empty == False:
                  ...
                  res2.loc[:,'material'] = k
                  res2.loc[:,'scenario'] = l 
                  res2.loc[:,'coarse_grinding'] = m
                  res2.loc[:,'distance to recycling facility'] = n
                  res2.loc[:,'distance to cement plant'] = p                   

               else:
                   pass

               lci_dict[str(i) +' - '+j + ' - ' + k + ' - ' + l + ' - ' + m + ' - ' + str(n) + ' - ' + str(p)] = res
               lciflow_dict[str(i) +' - '+j + ' - ' + k + ' - ' + l + ' - ' + m + ' - ' + str(n) + ' - ' + str(p)] = res2
...
def model_celavi_lci():
    ...
    f_d = pd.read_csv('foreground_results_processed_multi.csv')
    ...
    #final_product_result = pd.DataFrame()
    #Running LCA for all years as obtained from CELAVI
    processes = []
    manager = multiprocessing.Manager()
    lci_dict = manager.dict()
    lciflow_dict = manager.dict()
                        ...
                        for m in coarse_grind:
                            ...
                            for n in dist1:
                                ...
                                for p in dist2:
                                    ...
                                    final_dem = uslci_product_df.merge(fd_cur8, left_on = 0, right_on = 'flow name', how = 'left')
                                    final_dem = final_dem.fillna(0)
                                    #To make the optimization easier
                                    F = final_dem['flow quantity']/final_demand_scaler
                                    p = multiprocessing.Process(target = runner, args = (tech_matrix, F,i,j,k,l,m,n,p,final_demand_scaler,lci_dict,lciflow_dict))
                                    processes.append(p)
                                    p.start() 
                                    #runner(tech_matrix, F,i,j,k,l,m,n,p,final_demand_scaler,lci_dict,lciflow_dict)
 
    
    for process in processes:
          process.join()
    return  lci_dict,lciflow_dict    
    #return None,None
tim1 = time.time()  
lci_dicts,lciflow_dicts  = model_celavi_lci()

lci_result = pd.DataFrame()
lci_flow = pd.DataFrame()
for key in lci_dicts.keys():
    lci_result = pd.concat([lci_result,lci_dicts[key]])
    lci_flow = pd.concat([lci_flow,lciflow_dicts[key]])
    



lci_result.to_csv('lci_temp.csv', header=False, index = False)
lci_flow.to_csv('lciflow_temp.csv', header=False, index = False)
